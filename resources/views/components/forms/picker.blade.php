...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    <div x-data="{
        ...
        insertMedia(event) {
            ...
            this.loading = true;
            $wire.$set(event.detail.statePath, event.detail.media).then(() => {
                this.loading = false;
            });
        },
    }" x-on:insert-content.window="insertMedia($event)" class="curator-media-picker w-full">
        <template x-if="loading">
            <div class="flex flex-row gap-2 mt-4 ml-1 text-primary-500">
        ...
        <ul @class([
            ...
            'flex items-center gap-6 flex-wrap' =>
                $itemsCount <= 3 && !$shouldDisplayAsList,
            'curator-grid-container' => $itemsCount >= 3 && !$shouldDisplayAsList,
            'overflow-hidden bg-white border border-gray-300 rounded-lg shadow-sm divide-y divide-gray-300 dark:border-gray-700 dark:text-white dark:divide-gray-700 dark:bg-white/5' =>
                $itemsCount > 0 && $shouldDisplayAsList,
        ]) x-sortable
                        ...
                        <div class="w-full flex items-center gap-4 text-xs pe-2">
                            <div class="curator-picker-list-preview flex-shrink-0 h-12 w-12 checkered">
                                @if (str($item['type'])->contains('image'))
                                    <img src="{{ $item['thumbnail_url'] }}" alt="{{ $item['alt'] ?? $item['name'] }}"
                                ...
                                @else
                                    <x-curator::document-image label="{{ $item['name'] }}" type="{{ $item['type'] }}"
                                        extension="{{ $item['ext'] }}" icon-size="md" />
                                @endif
                            </div>
                        ...
                        <div @class([
                            ...
                            @elseif (str($item['type'])->contains('video'))
                                <video controls src="{{ $item['url'] }}"
                                    @if ($shouldLazyLoad()) preload="none" @endif></video>
                            @else
                            ...
                            <div class="absolute top-0 right-0">
                                <div
                                    class="relative flex items-center bg-gray-950 divide-x divide-gray-700 rounded-bl-lg shadow-md">
                                    @if ($isMultiple)
                                        <div x-sortable-handle
        ...
        <div @class(['flex items-center gap-4', 'mt-4' => $itemsCount > 0])>
