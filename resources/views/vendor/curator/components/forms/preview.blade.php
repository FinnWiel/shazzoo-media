...
@if ($record)
    <div>
        @if (str($record->type)->contains('image'))
            <img src="{{ $record->url }}" alt="{{ $record->alt }}" width="{{ $record->width }}"
                height="{{ $record->height }}" loading="lazy"
        ...
        @else
            <x-curator::document-image label="{{ $record->name }}" icon-size="xl" class="p-4 rounded" :type="$record->type"
